...
function AquaGauge(target) {
    ...
    function drawCurrentValue(cvs, currValue, props) {
        ...
        roundRect(cvs.ctx, cvs.cX - getRelVal(cvs, 40), cvs.height * 0.66, getRelVal(cvs, 80), getRelVal(cvs, 35), getRelVal(cvs, 5), true, true);
        cvs.ctx.font = props.dialValueTextFont;
        cvs.ctx.fillStyle = props.dialValueTextColor;
        adj = cvs.ctx.measureText(currValue).width / 2;
        cvs.ctx.fillText(currValue, cvs.cX - adj, cvs.height * 0.75);
        cvs.ctx.restore();
    }
...
function GaugeParams(cvs) {
    ...
    this.dialTitleTextFont = "bold " + getRelVal(cvs, 30) + "px Trebuchet MS";
    this.dialTitleTextColor = "#454545";
    this.dialTitle = "ZZ";//TITLU GAUGE
    this.dialSubTitleTextFont ="bold " + getRelVal(cvs, 29) + "px Trebuchet MS";
    this.dialSubTitleTextColor = "#757575";    
    this.dialSubTitle = "XX";
    this.showGlossiness = true;

    //Scale
    this.minValue = 0.0;
    this.maxValue = 100.0;
    this.noOfDivisions = 5.0;
    ...
    //Scale Ranges !!!!AICI CULORI
    this.rangeSegments = [{ start: 0, end: 40, color: "yellow" },
                          { start: 40, end: 90, color: "greenyellow" },
                          { start: 90, end: 100, color: "red"}];

    //Scale - Text
    this.dialScaleFont = "bold " + getRelVal(cvs, 24) + "px Trebuchet MS";
    this.dialSubScaleFont = "bold " + getRelVal(cvs, 19) + "px Trebuchet MS";    
